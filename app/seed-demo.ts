 ...
 *
 *   • Mock customers ship with the literal password "password".
 ...
 *   • kitchen_staff       (one "Demo Cook", PIN 9012)
 */
...
async function customersEmpty(): Promise<boolean> {
...
try {
  ...
  // ── Menu items ───────────────────────────────────────────────────────────
  if (await tableEmpty("menu_items")) {
  ...
  if (await tableEmpty("kitchen_staff")) {
