...
import { UtensilsCrossed, ArrowLeft, ChevronLeft, Loader2, Eye, EyeOff } from "lucide-react";
...
export default function WaiterLoginPage() {
  ...
  async function submit(ev?: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json() as { ok: boolean; waiter?: unknown; error?: string };
      if (data.ok) {
        // Mirror the profile into sessionStorage so the /waiter page header
        // renders instantly without waiting on /api/auth/waiter/me.
        try { sessionStorage.setItem("waiter_session", JSON.stringify(data.waiter)); } catch { /* ignore */ }
        router.replace("/waiter");
      } else {
  ...
  function selectStaff(w: WaiterTile) {
    setTarget(w);
    setPassword("");
    setError("");
    setStep("password");
