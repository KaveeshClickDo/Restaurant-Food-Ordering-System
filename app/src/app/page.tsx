...
function OrdersView({
  ...
  onSetScreen: (s: string) => void;
  onTrackOrder: (order: Order) => void;
  onOpenAuth: () => void;
}) {
  const { currentUser, addToCart, refreshCurrentUser } = useApp();
        ...
        <div className="flex-1 overflow-y-auto pb-28 lg:pb-8">
          ...
          {screen === "orders" && (
            <OrdersView
              ...
              onOpenAuth={() => setAuthModal({ open: true, tab: "login" })}
            />
          )}
          ...
          {/* Render SiteFooter below all active screens */}
          <div className="mt-8">
            <SiteFooter />
          </div>
        </div>
