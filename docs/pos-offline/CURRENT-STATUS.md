...
| Background sync worker | **Decided 2026-06-24: not doing closed-app background sync.** The outbox is in **encrypted SQLite** a native worker can't read; the committed `OutboxSyncWorker.kt` is **stale** (reads SharedPreferences `pos_outbox`, never written anymore) and is **not scheduled** — leave inert / rewrite for SQLite only if ever revisited. Instead, queued sales **sync on app open AND on warm resume** (`pos/page.tsx`: cold-open drain + `visibilitychange` resume drain, gated on connectivity so it never burns retry budget offline). Fine for an always-on POS tablet. |
